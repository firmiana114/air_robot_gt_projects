...
function canStartGuide(data){
  var state=guideStateValue(data);
  return data&&data.nav_bridge&&data.nav_bridge.ready&&(state==='qa_listening'||(state==='unknown'&&data.workflow&&data.workflow.ready)||data.no_robot_mode);
}
function servicesReady(data){
  var state=guideStateValue(data);
  return data&&data.main_loop==='running'&&data.nav_bridge&&data.nav_bridge.ready&&(state==='qa_listening'||(state==='unknown'&&data.workflow&&data.workflow.ready)||data.no_robot_mode);
}
...
function renderStatus(data){
  ...
  var workflowLabel=(guideState==='unknown'&&data.workflow&&data.workflow.status)?data.workflow.status:guideStateLabel(guideState);
  setText('overall',servicesReady(data)?(data.no_robot_mode?'无机器人模式就绪':'全部就绪'):(data.nav_bridge.ready?workflowLabel:'导航未就绪'));
  ...
  setText('workflow',workflowLabel);
...
function addHotRow(row){
  ...
  actionGroup.className='hot-row-actions';
  actionGroup.appendChild(currentPoseBtn);
...
def clear_current_runtime_log(path: Path, reason: str) -> None:
    try:
        ...
        logger.warning(
            "清空当前运行日志失败：path=%s, reason=%s, error_type=%s, error=%s",
            path,
            reason,
            type(exc).__name__,
            exc,
...
def _restart_service_container_bg(container_name: str, service_key: str, docker_path: Path) -> None:
    # docker restart 可能阻塞十几秒，放后台避免 HTTP 请求长时间挂起。
    try:
        restart_service_container(container_name, docker_path=docker_path)
    except CommandError as exc:
        logger.error(
            "后台重启服务容器失败：service=%s, container=%s, error_type=%s, error=%s",
            service_key,
            container_name,
            type(exc).__name__,
            exc,
...
def _guide_state_payload(config: ConsoleConfig, workflow_ready: bool) -> dict:
    guide_state = read_guide_state(config.guide_state_file)
    if guide_state.state == "unknown" and workflow_ready:
        return {
            "state": "qa_listening",
            "run_id": "",
            "detail": "由当前 workflow ready 状态兼容推断",
            "time": "",
        }
    return guide_state.to_dict()
...
def create_app(config: ConsoleConfig | None = None) -> FastAPI:
    ...
    def status() -> dict:
        ...
        return {
            ...
            "guide_state": _guide_state_payload(config, workflow.ready),
